...
La fréquence est de 96 car les donnée sont ont été mesuré toutes les 15min
```{r}
...
plot(elec)
```
Il semble y avoir une saisonalité dans la consommation électrique,
il ne semble pas qu'il existe une tendance, car la consommation électrique ne semble ni augmneter ni diminiuer.


Division des des donnes en echantillons d'appre,tissage et de test. L'échantillon de test représente le dernier jour de consommation.
```{r}
...
```
```{r}
ggtsdisplay(diff(elec_train,lag = 96))
